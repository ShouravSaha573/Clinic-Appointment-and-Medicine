...
export const useAppointmentStore = create((set, get) => ({
  ...
  bookAppointment: async (appointmentData) => {
    ...
    try {
      ...
      get().getUserAppointments(get().lastUserAppointmentsFilters || {});
  ...
  getUserAppointments: async (filters = {}) => {
    ...
    try {
      ...
      set({ 
        ...
        lastUserAppointmentsFilters: filters,
        pagination: res.data.pagination,
        isLoading: false 
      });
    } catch (error) {
  ...
  cancelAppointment: async (id) => {
    try {
      ...
      // Update local list immediately (avoid stale status in UI)
      set((state) => ({
