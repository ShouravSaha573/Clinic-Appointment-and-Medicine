...
const BookAppointmentPage = () => {
  ...
  const doctorIdFromQuery = new URLSearchParams(location.search).get("doctor");
  const effectiveDoctorId = doctorIdFromParams || doctorIdFromQuery;
  ...
  useEffect(() => {
    fetchDoctors();
    if (effectiveDoctorId && !initialDoctor) {
      fetchDoctorById(effectiveDoctorId);
    }
  }, [effectiveDoctorId, initialDoctor]);
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        if (selectedTime) query.set("time", selectedTime);

        const response = await axiosInstance.get(`/external/weather?${query.toString()}`);
        setBookingWeather(response.data);
      } catch (error) {
        console.error("Error fetching booking weather:", error?.response?.data || error);
        setBookingWeather(null);
        setWeatherError(
          error?.response?.data?.message || "Weather forecast is unavailable right now."
        );
      } finally {
  ...
  const getWeatherLabel = (code) => {
    ...
    if (!Number.isFinite(safeCode)) return "Forecast";
    if (safeCode === 0) return "Clear";
    if ([1, 2, 3].includes(safeCode)) return "Partly cloudy";
    if ([45, 48].includes(safeCode)) return "Fog";
    if (safeCode >= 51 && safeCode <= 57) return "Drizzle";
    if (safeCode >= 61 && safeCode <= 67) return "Rain";
    if (safeCode >= 71 && safeCode <= 77) return "Snow";
    if (safeCode >= 80 && safeCode <= 82) return "Rain showers";
    if (safeCode >= 95 && safeCode <= 99) return "Thunderstorm";
    return `Weather code ${safeCode}`;
  };
              ...
              {Array.isArray(doctors) && doctors.map((doctor) => (
                ...
                >
                  <div className="flex items-center mb-4">
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mr-4 overflow-hidden border border-gray-200 bg-gradient-to-r from-blue-500 to-cyan-500">
                      {doctor?.profileImage ? (
                        <img
                          src={getOptimizedAvatarSrc(doctor.profileImage)}
                          alt={doctor?.name ? `Dr. ${doctor.name} profile` : "Doctor profile"}
          ...
          <div>
            <div className="bg-white rounded-lg shadow-sm border p-6 mb-6">
              <div className="flex items-center mb-6">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mr-4 overflow-hidden border border-gray-200 bg-gradient-to-r from-blue-500 to-cyan-500">
                  {selectedDoctor?.profileImage ? (
                    <img
                      src={getOptimizedAvatarSrc(selectedDoctor.profileImage)}
                      alt={selectedDoctor?.name ? `Dr. ${selectedDoctor.name} profile` : "Doctor profile"}
                      className="w-full h-full object-cover"
                      loading="lazy"
                      decoding="async"
                    />
                ...
                <div className="mt-8">
                  ...
                  <div className="mt-6 bg-gray-50 rounded-lg border p-4">
                    <div className="flex items-center justify-between gap-4 mb-2">
                      <div>
                        <h4 className="text-sm font-semibold text-gray-800">Weather for selected date/time</h4>
                        <p className="text-xs text-gray-500">Source: Open-Meteo (external API)</p>
                      </div>
                      {isWeatherLoading && <span className="loading loading-spinner loading-sm"></span>}
                    </div>

                    {weatherError ? (
                      <div className="text-sm text-red-600">{weatherError}</div>
                    ) : bookingWeather?.daily ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="bg-white rounded-lg p-3 border">
                          ...
                          <div className="text-sm text-gray-800">
                            <div>
                              ...
                              {getWeatherLabel(bookingWeather.daily.weatherCode)}
                            </div>
                            <div>
                              <span className="font-medium">Temp:</span>{" "}
                              {bookingWeather.daily.tempMin ?? "-"}–{bookingWeather.daily.tempMax ?? "-"}{" "}
                              {bookingWeather.units?.temperature || "°C"}
                            </div>
                            <div>
                              <span className="font-medium">Precip:</span>{" "}
                              {bookingWeather.daily.precipitationSum ?? "-"}{" "}
                              {bookingWeather.units?.precipitation || "mm"}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg p-3 border">
                          ...
                          {!selectedTime ? (
                            <div className="text-sm text-gray-600">Select a time slot to see hourly forecast.</div>
                          ) : bookingWeather?.hourlySelected ? (
                            <div className="text-sm text-gray-800">
                              <div>
                                <span className="font-medium">Hour:</span>{" "}
                                {bookingWeather.hourlySelected.time || selectedTime}
                              </div>
                              <div>
                                <span className="font-medium">Condition:</span>{" "}
                                {getWeatherLabel(bookingWeather.hourlySelected.weatherCode)}
                              </div>
                              <div>
                                <span className="font-medium">Temp:</span>{" "}
                                {bookingWeather.hourlySelected.temperature ?? "-"}{" "}
                                {bookingWeather.hourlyUnits?.temperature || "°C"}
                              </div>
                              <div>
                                <span className="font-medium">Rain:</span>{" "}
                                {bookingWeather.hourlySelected.precipitationProbability ?? "-"}{" "}
                                {bookingWeather.hourlyUnits?.precipitationProbability || "%"}
                              </div>
                              <div>
                                <span className="font-medium">Wind:</span>{" "}
                                {bookingWeather.hourlySelected.windSpeed ?? "-"}{" "}
                                {bookingWeather.hourlyUnits?.wind || "km/h"}
                              </div>
                            </div>
                          ) : (
                            <div className="text-sm text-gray-600">No hourly forecast available.</div>
                          )}
                        </div>
                      </div>
                    ) : (
                      <div className="text-sm text-gray-600">Select a date to load forecast.</div>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b pb-4">
                  ...
                  <span className="flex items-center gap-2">
                    <span className="w-8 h-8 rounded-full overflow-hidden border border-gray-200 bg-gradient-to-r from-blue-500 to-cyan-500 flex items-center justify-center">
                      {selectedDoctor?.profileImage ? (
                        <img
                          src={getOptimizedAvatarSrc(selectedDoctor.profileImage)}
                          alt={selectedDoctor?.name ? `Dr. ${selectedDoctor.name} profile` : "Doctor profile"}
