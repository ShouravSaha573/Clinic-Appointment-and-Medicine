...
const AppointmentDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        // Backend defaults to Dhaka if lat/lon not provided.
        const response = await axiosInstance.get(
          `/external/weather?date=${encodeURIComponent(appointment.appointmentDate)}`
        );

        setWeather(response.data);
      } catch (error) {
        console.error("Failed to fetch weather:", error?.response?.data || error);
        setWeather(null);
        setWeatherError(
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`badge ${statusStyles[safeStatus] || "badge-warning"} badge-lg`}>
  ...
  const normalizedStatus = String(appointment?.status || "pending").toLowerCase().trim();
  const effectiveStatus =
    normalizedStatus === "confirmed" && !appointment?.doctorResponse?.respondedAt
      ? "pending"
      : normalizedStatus;
        ...
        <div className="flex items-center gap-2">
          {isOverdue && effectiveStatus === "pending" ? (
            <>
              ...
              <span className="badge badge-error badge-lg">Overdue</span>
            </>
          ) : (
            <>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            {isOverdue && (
              <div className="mt-4 alert alert-warning">
                ...
                <span className="text-sm">
                  This appointment has passed the scheduled time (over 1 hour) and is not completed yet.
                </span>
              </div>
            )}
            ...
            <div className="mt-6 pt-4 border-t">
              <div className="flex items-center justify-between gap-4 mb-2">
                <div>
                  <h3 className="text-sm font-semibold text-gray-800">Weather on appointment day</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <div className="text-sm font-medium text-gray-800">
                      {getWeatherLabel(weather.daily.weatherCode)}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="text-xs text-gray-500">Temperature</div>
                    <div className="text-sm font-medium text-gray-800">
                      {weather.daily.tempMin ?? "-"}–{weather.daily.tempMax ?? "-"} {weather.units?.temperature || "°C"}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="text-xs text-gray-500">Precipitation</div>
                    <div className="text-sm font-medium text-gray-800">
                      {weather.daily.precipitationSum ?? "-"} {weather.units?.precipitation || "mm"}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="text-xs text-gray-500">Wind (max)</div>
                    <div className="text-sm font-medium text-gray-800">
                      {weather.daily.windSpeedMax ?? "-"} {weather.units?.wind || "km/h"}
                    </div>
                  </div>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-lg shadow-sm border p-6">
              ...
              {appointment.notes && (
                <div className="mb-4">
                  ...
                  <p className="text-gray-700 mt-1 p-3 bg-gray-50 rounded-md whitespace-pre-line">
                    {appointment.notes}
                  </p>
                </div>
              )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="text-center text-sm font-medium text-gray-700 pt-2">
                Pay After Consulting Doctor. Thank you
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div className="space-y-3">
              {effectiveStatus === "pending" && !isOverdue && (
                <>
                  <button
