...
export const createDoctor = async (req, res) => {
  try {
    ...
    let profileImage = "";
    const raw = typeof req.body.profileImage === "string" ? req.body.profileImage.trim() : "";
    if (raw) {
      const isDataUrl = raw.startsWith("data:image/");

      if (hasCloudinaryConfig) {
        const uploadedResponse = await cloudinary.uploader.upload(raw, {
          folder: "clinic-app/doctor-profile-images",
        });
        profileImage = uploadedResponse.secure_url;
      } else {
        if (isDataUrl && raw.length > 8_000_000) {
          return res.status(413).json({ message: "Profile picture is too large. Please upload a smaller image." });
        }
        // If it's an http(s) URL, store as-is; if it's a small data URL, store as-is.
        profileImage = raw;
      }
    }
